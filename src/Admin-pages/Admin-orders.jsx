  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-gray-100">
      ...
      <main className="flex-1 p-4 md:p-8 overflow-y-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 sm:gap-0 mb-4 md:mb-6 mt-1">
          <h1 className="text-xl md:text-2xl font-semibold text-gray-900">Manage Orders</h1>
          <div className="flex items-center gap-2 bg-black text-white px-4 md:px-5 py-1.5 md:py-2 rounded-full text-xs md:text-sm self-start sm:self-auto">
          ...
          <div className="space-y-6">
            {filteredOrders.map((order) => {
              ...
              return (
                ...
                >
                  <div className="flex flex-col sm:flex-row justify-between sm:items-start gap-2 sm:gap-0 mb-4">
                    <div>
                      <h3 className="font-medium text-gray-800">Order #{orderId}</h3>
                      <p className="text-xs md:text-sm text-gray-600 mt-1">User ID: {userId}</p>
                  ...
                  <div className="divide-y">
                    {products.map((item, i) => {
                      ...
                      return (
                        <div key={i} className="flex flex-col sm:flex-row sm:items-center justify-between py-3 gap-2 sm:gap-0">
                          <div className="flex items-center gap-3 min-w-0">
                            {img ? (
                              <img src={img} alt={name} className="w-12 h-12 md:w-16 md:h-16 object-cover rounded-md flex-shrink-0" />
                            ) : (
                              <div className="w-12 h-12 md:w-16 md:h-16 bg-gray-200 rounded-lg flex items-center justify-center text-gray-400 text-[10px] md:text-xs flex-shrink-0">No Img</div>
                            )}
                            <div className="min-w-0">
                              <p className="font-medium text-sm md:text-base text-gray-900 truncate max-w-[150px] sm:max-w-none">{name}</p>
                              <p className="text-xs md:text-sm text-gray-600 truncate">Product ID: {productId}</p>
                              <p className="text-xs md:text-sm text-gray-600">Quantity: {quantity}</p>
                            </div>
                          </div>
                          <span className="text-gray-700 text-sm font-semibold self-end sm:self-auto">₹{price}</span>
                        </div>
                      );
                    })}
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between mt-5 gap-3 sm:gap-0">
                    <div className="flex items-center gap-2 md:gap-3">
                      <span className="font-medium text-sm md:text-base text-gray-800">Status:</span>
                      {status === "Cancelled" ? (
                        <span className="text-red-600 font-bold text-xs md:text-sm bg-red-100 px-2 py-0.5 rounded-md shadow-sm">
                          Cancelled
                        </span>
                      ) : (
                        <select
                          ...
                          className={`border border-gray-300 rounded-md text-xs md:text-sm px-2 md:px-3 py-1 focus:outline-none transition ${
                    ...
                    <div className="self-end sm:self-auto">
                      <span className="font-medium text-sm md:text-base text-gray-800 mr-2">Total:</span>
                      <span className="font-semibold text-sm md:text-base text-gray-900">₹{totalAmount}</span>
                    </div>
                  </div>
                </div>
              );
